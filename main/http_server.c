...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        // httpd_register_uri_handler(server, &hello); // start method GET
        httpd_register_uri_handler(server, &echo); // start method POST
        // httpd_register_uri_handler(server, &ctrl); // start method PUT
        #if CONFIG_BASIC_AUTH
